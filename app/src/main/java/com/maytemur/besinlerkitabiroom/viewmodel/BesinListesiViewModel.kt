...
class BesinListesiViewModel(application: Application) : BaseViewModel(application) {
    ...
    private var guncellemeZamani =
        10 * 60 * 1000 * 1000 * 1000L //dakikanin nano time a çevrilmiş hali
...
//                            besinYukleniyor.value = false
    ...
    private fun sqliteSakla(besinListesi: List<Besin>) {
        ...
        launch {
            ...
            dao.deleteAllBesin()
            val uuidListesi =
                dao.insertAll(*besinListesi.toTypedArray())  // başına yıldız koyunca verdiğimiz listeyi tek tek veriyor
            var i = 0
            while (i < besinListesi.size) {
                besinListesi[i].uuid =
                    uuidListesi[i].toInt()//sqlite oluşturulan id leri modelin içine koyuyoruz burada
                i = i + 1
            }
            besinleriGoster(besinListesi)
        }
